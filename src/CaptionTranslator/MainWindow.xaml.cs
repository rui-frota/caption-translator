...
public partial class MainWindow : Window
{
    ...
    private async void CaptureService_FrameCaptured(object? sender, DrawingBitmap frame)
    {
        ...
        try
        {
            ...
            _pendingTranslation.Append(newSourceSegment);
            if (!ShouldTranslatePending(_pendingTranslation.Text)
                || DateTime.UtcNow < _nextTranslationAttemptUtc)
            {
                return;
            }

            var pendingText = _pendingTranslation.Text;
    ...
    private static bool ShouldTranslatePending(string text)
    {
        var wordCount = text.Split(' ', StringSplitOptions.RemoveEmptyEntries).Length;
        return wordCount >= 3 || text.Contains('.', StringComparison.Ordinal) || text.Contains('?', StringComparison.Ordinal);
    }
}